...
async function paso_4b() {
    await new Promise(resolve => {
        ...
        frappe.call({ 
            ...
            callback: function(r){
                resolve(
                    paso_4cantidadRasgosDirectosEvaluacion = r.message.length
                )
...
async function paso_5(){
    await paso_5a()
    await paso_5b()
    await paso_5c(paso_5acumulador, 3000)
}
...
// MODIFICACION 29-01-23 - SE CAMBIO DE LUGAR EL RESOLVE
async function paso_5a() {
    await new Promise(resolve => {
        console.log("PASO 5A"),
        frappe.call({ 
...
// MODIFICACION 29-01-23 - SE BORRO EL "i" DENTRO DEL FOREACH
async function paso_5c(veces, tiempo){
    for(let i=0; i < veces; i++){
        await new Promise(resolve => {
        console.log("PASO 5C"),
        resolve(
            paso_5lista.forEach(function(element){
                if(element["idx"] >= paso_5i1 && element["idx"] < paso_5i2){
                    console.log(element["idx"])
                    frappe.call({ 
                        method:"frappe.client.insert",
        ...
        await new Promise(resolve => {
                paso_6lista.forEach(function(v, i) {
                    if(i >= paso_6i1 && i < paso_6i2){
                        ...
                        resolve(
                            console.log("PASO 6 SUPERYO CONTINUA")
                        )
                    }
                }),
                paso_6i1 += 100,
        ...
        await new Promise(resolve => {
            paso_6lista.forEach(function(v, i) {
                if(i >= paso_6i1 && i < paso_6i2){
                    ...
                    resolve(
                        console.log("PASO 6 ELLO CONTINUA")
                    )
                }
            }),
            paso_6i1 += 100,
        ...
        await new Promise(resolve => {
            paso_6lista.forEach(function(v, i) {
                if(i >= paso_6i1 && i < paso_6i2){
                    ...
                    resolve(
                        console.log("PASO 6 SIMISMO CONTINUA")
                    )
                }
            }),
            paso_6i1 += 100,
...
// MODIFICACION 29-01-23 - SE CAMBIA POSICION DE RESOLVE
/*
async function paso_8(tiempo) {
    await new Promise(resolve => {
        console.log("PASO 8A"),
