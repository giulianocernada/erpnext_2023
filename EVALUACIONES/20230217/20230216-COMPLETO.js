...
async function paso_6comprobacion_c() {
    await new Promise(resolve => {
        if(paso_6rasgosDirectosRealizados.length != paso_6rasgosDirectosPlanificados.length){
            resolve(
                paso_6comprobacionRealizada = true,
                paso_6rasgosDirectosPlanificados.forEach(function(element){
                    if(paso_6rasgosDirectosRealizados.some(x => x.nombre === element["nombre"])){
                    } else {
                        let paso_6rasgo = element["nombre"]
                        paso_6rasgo = paso_6rasgo.split(" ")
                        if(paso_6rasgosFaltantes.includes(paso_6rasgo[0])){

                        } else {
                            paso_6rasgosFaltantes.push(paso_6rasgo[0])
                        }
                    }
                })
            )
        ...
        console.log(paso_6rasgosFaltantes)
    })
}
...
async function paso_6comprobacion_d() {
    await new Promise(resolve => {
        if(paso_6comprobacionRealizada === true){
            paso_6rasgosFaltantes.forEach(function(element){
                    frappe.call({ 
                        method:"frappe.client.get_list", 
                        args: { 
                            doctype: "Rasgos de Personas",
                        ...
                        callback: function(r) {
                            resolve(
                                paso_6listaRasgosFaltantes.push(r.message[0].name)
                            )
                        }
                    })
                }
...
async function paso_6comprobacion_e() {
    return new Promise(resolve => {
        if(paso_6comprobacionRealizada === true){
            resolve(
                paso_6comprobacionDocumentoActual = cur_frm.doc.name
            )
        } else {
            resolve(
                console.log("sigue")
...
async function paso_6comprobacion_f(lista, tiempo) {
    if(paso_6comprobacionRealizada === true){
        for(let doc in lista){
            await new Promise(resolve => {
                resolve(
            ...
            await paso_6comprobacion_g(1000)
            await paso_6comprobacion_h()
            await guardar(tiempo)
        }
    } else {
        await new Promise(resolve => {
...
async function paso_6comprobacion_g(tiempo) {
    await new Promise(resolve => {
        resolve(
...
async function paso_6comprobacion_h() {
    await new Promise(resolve => {
        resolve(
            cur_frm.set_value("estado", "ANALIZADO")
        )
...
async function paso_6comprobacion_i(origen, tiempo) {
    if(paso_6comprobacionRealizada === true){
        await new Promise(resolve => {
            resolve(
                frappe.set_route("Form", "Evaluaciones", origen)
...
async function paso_6comprobacion_j(tiempo) {
    await new Promise(resolve => {
        if(paso_6comprobacionRealizada === true){
            resolve(
                paso_6(3000)
